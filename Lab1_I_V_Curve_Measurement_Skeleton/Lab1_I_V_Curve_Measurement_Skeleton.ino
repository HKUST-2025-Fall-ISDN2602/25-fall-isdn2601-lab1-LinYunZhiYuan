...
void setup() {
  ...
  for (int onPeriod = 0; onPeriod < 256; onPeriod++) 
    ...
    x = float(onPeriod)/255 ; // calculate duty cycle for TASK 1 or the voltage at Vout in mV for Task 2 3 4
    y = Vout_value*3.3/4095; // calculate the voltage at Vout in V for TASK 1 or current in uA for Task 2 3 4
    Serial.print(x); 
