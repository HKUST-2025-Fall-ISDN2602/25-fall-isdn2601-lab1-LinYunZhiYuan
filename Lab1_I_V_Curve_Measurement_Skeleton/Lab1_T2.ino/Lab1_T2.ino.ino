...
float R = 2000 ; //*** Resistance in Mohms ***
...
void setup() {
  ...
  for (int onPeriod = 0; onPeriod < 256; onPeriod++) 
    ...
    x = (c2-c1)*1000 ; // calculate the voltage at Vout in mV
    y = (c2-c1)/22 ; // calculate current in uA for Task 2 3 4 
    Serial.print(y); // print the voltage at Vout in mV
    Serial.print(","); 
    Serial.println(x);// print the current in uA  
  }
